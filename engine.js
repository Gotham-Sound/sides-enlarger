...
(function (root, factory) {
    ...
    for (const P of pages) {
      ...
      for (const L of P.lines) {
        ...
        if (SLUG_RE.test(text)) {
          ...
          push({ t: 'slug', text: text + (sceneNum ? '     Sc. ' + sceneNum : '') });
          continue;
        }
  ...
  return function createSidesEngine({ pdfjsLib, PDFLib }) {
    ...
    return {
      ...
      // The local name gate, exported (pure, no behavior change) so
      // tools/conformance_check.mjs can measure it against the hub's cue_gate
      // and charset vectors (scriptparse #40 Phase-1 sizing). It is NOT a
      // policy interpreter; Phase-1 adoption retires it onto policy.json.
      isPlausibleName,
    };
  };
});
