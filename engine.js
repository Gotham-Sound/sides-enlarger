...
(function (root, factory) {
  ...
  // Reconstruct the text of rotated runs (per-recipient watermarks) on a
  // page: cluster items on the same rotated baseline (same angle, same
  // perpendicular offset), order them along it, and join with a space only
  // across a word-sized gap (glyph-per-op stamps abut). Reader mode drops the
  // rotated items from the reading text and carries this text in its footer
  // instead, so the recipient's stamp survives the reflow.
  function rotatedStampTexts(rotItems) {
    const pts = rotItems.map(it => {
      const c = Math.cos(it.ang), s = Math.sin(it.ang);
      return { it, ang: it.ang, along: it.x * c + it.y * s, perp: -it.x * s + it.y * c };
    }).sort((p, q) => (p.ang - q.ang) || (p.perp - q.perp));
    const texts = [];
    let cluster = [];
    const flush = () => {
      if (!cluster.length) return;
      cluster.sort((p, q) => p.along - q.along);
      let text = '', end = null;
      for (const p of cluster) {
        const gap = end === null ? 0 : p.along - end;
        text += (end !== null && gap > Math.max(1.5, 0.25 * (p.it.size || 12)) ? ' ' : '') + p.it.str;
        end = p.along + (p.it.w || 0);
      }
    ...
    for (const p of pts) {
      ...
      if (last && (Math.abs(p.ang - last.ang) > 0.05 || Math.abs(p.perp - last.perp) > 2.0)) flush();
      cluster.push(p);
    }
    flush();
    return texts;
  ...
  function buildReaderElements(pages) {
    const els = [];
    // every element carries its source page (0-based index + printed label)
    // and the scene it sits in (id from the margin scene number, heading
    // normalized), so a consumer can key a cursor to a page or a scene
    // without re-deriving either. Additive: the PDF renderer ignores them.
    let curPage = null, curLabel = null, curScene = { id: null, heading: null };
    const push = el => {
      if (!(el && el.text && el.text.trim())) return;
      el.page = curPage; el.pageLabel = curLabel;
    ...
    for (const P of pages) {
      ...
      for (const L of P.lines) {
        ...
        // detect it there, then drop it and its matching right-margin twin so
        // neither bleeds into the reflowed text. Slugs surface it as "Sc. N".
        let sceneNum = '';
        {
          const xs = [...live].sort((a, b) => a.x - b.x);
          if (xs.length >= 2 && /^[A-Z]{0,3}\d+[A-Z0-9]*$/.test(xs[0].str.trim())
              && xs[1].x - (xs[0].x + xs[0].w) > 16) {
            sceneNum = xs[0].str.trim();
            const drop = new Set([xs[0]]);
            const last = xs[xs.length - 1];
            if (last !== xs[0] && last.str.trim() === sceneNum) drop.add(last);
            live = live.filter(t => !drop.has(t));
          }
        }
        ...
        if (SLUG_RE.test(text)) {
          ...
          // "- DAY     Sc. 24" can't read as the phrase "DAY 24".
          curScene = { id: sceneNum || null, heading: normalizeHeading(text) };
          push({ t: 'slug', text: text + (sceneNum ? '     Sc. ' + sceneNum : ''), heading: curScene.heading });
          continue;
        }
  ...
  // text contrast. Sixteen entries; the first eight are the original set and
  // their indices never move, since saved preferences and .sceneline files
  // refer to them.
  const PALETTE = [
    ...
    // eight more (v1.14.0, a mixer's day with more than eight speaking parts):
    // the same luminance floor, hues spaced between the first eight. No neutral
    // gray: an even grey fill is how sides mark omitted text, and the
    // grey-region detector (and check.py's mirror of it) would take a gray
    // highlight for one. Every entry keeps a component spread above 0.05.
    { key: 'coral',      hex: '#FFD9CD', rgb: [1.000, 0.851, 0.804] },
    { key: 'rose',       hex: '#FFD8E1', rgb: [1.000, 0.847, 0.882] },
    { key: 'orchid',     hex: '#F3D9F6', rgb: [0.953, 0.851, 0.965] },
    { key: 'periwinkle', hex: '#DDE1FC', rgb: [0.867, 0.882, 0.988] },
    { key: 'aqua',       hex: '#C9F1EC', rgb: [0.788, 0.945, 0.925] },
    { key: 'lime',       hex: '#E7F6AE', rgb: [0.906, 0.965, 0.682] },
    { key: 'gold',       hex: '#FFE79C', rgb: [1.000, 0.906, 0.612] },
  ...
  return function createSidesEngine({ pdfjsLib, PDFLib, policy }) {
    ...
    async function extract(bytes) {
      ...
      for (let p = 1; p <= doc.numPages; p++) {
        ...
        const items = tc.items
          ...
          .map((it, seq) => {
            ...
            return {
              ...
              ang: Math.atan2(it.transform[1], it.transform[0]),   // baseline angle (0 = upright)
            };
          });
        ...
        pages.push({ index: p, width: vp.viewBox[2] - vp.viewBox[0], height: vp.viewBox[3] - vp.viewBox[1], lines: buildLines(bodyItems), items: bodyItems,
          rotatedItems: items.length - bodyItems.length,
          // the stamp's own words, so reader mode can carry them (see rotatedStampTexts)
          rotatedTexts: rotatedStampTexts(items.filter(it => it.rot)) });
      }
      await doc.destroy();
    ...
    async function renderReader(elements, requested, hlMap, report) {
      ...
      const newPage = () => {
        ...
        y = H - TOP;
        const note = 'READER MODE - reflowed text; page numbers do not match the shooting script';
        page.drawText(note, { x: MX, y: 30, size: 7.5, font: F, color: gray });
        const pn = String(readerPage);
        const pnX = W - MX - F.widthOfTextAtSize(pn, 9);
        page.drawText(pn, { x: pnX, y: 30, size: 9, font: F, color: gray });
        drawStamp(page, F.widthOfTextAtSize(note, 7.5), pnX);
      };
      // The recipient's watermark text, in the footer of every reader page
      // (report.readerStamps). On the footer line when it fits between the
      // note and the page number, else on its own line just above. A diagonal
      // re-draw across the page is the escalation if a studio ever asks for
      // more; it would replace this one helper.
      const stampText = sanitize((report.readerStamps || []).join(' · '));
      const drawStamp = (pg, noteW, pnX) => {
        if (!stampText) return;
        const sz = 8, dark = PDFLib.rgb(0.3, 0.3, 0.3);
        const sw = F.widthOfTextAtSize(stampText, sz);
        if (MX + noteW + 16 + sw + 12 <= pnX) pg.drawText(stampText, { x: pnX - 12 - sw, y: 30, size: sz, font: F, color: dark });
    ...
    async function process(bytes, opts = {}) {
      ...
      if (mode === 'reader') {
        ...
        // The recipient's stamp text rides every reader page's footer: rotated
        // runs (signal 1, reconstructed per page) and repeated-position groups
        // (signal 2, the burn-in rail). Distinct, in first-seen order.
        const stamps = [];
        for (const P of pages) for (const t of (P.rotatedTexts || [])) if (!stamps.includes(t)) stamps.push(t);
        for (const b of burn.rail) if (!stamps.includes(b.text)) stamps.push(b.text);
        report.readerStamps = stamps;
        if (stamps.length) report.warnings.push('Recipient watermark text carried into the footer of every reader page: ' + stamps.map(t => '“' + t + '”').join(', ') + '.');
        if (opts.emit === 'elements') {
          // the reader as DATA: the same element stream the PDF renderer
          // consumes, numbered, with each element's highlight resolved from
          // the requested map. No PDF is built; renderReaderPdf() turns this
          // exact array back into the Reader-mode PDF later.
          elements.forEach((el, i) => {
            el.i = i;
            const idx = el.name != null && hl[el.name] != null ? hl[el.name] : null;
            el.highlight = idx != null ? { palette: idx, key: PALETTE[idx].key, hex: PALETTE[idx].hex, rgb: PALETTE[idx].rgb } : null;
          });
    ...
    // reader(bytes, opts) -> { elements, report }: Reader mode as a function.
    // The reflowed script as an ordered element stream (break / slug / action
    // / cue / paren / dialogue / transition), each with speaker name, source
    // page, scene and resolved highlight, for consumers that draw the reader
    // themselves (Sides Helper). opts: { highlights, watermarkText }.
